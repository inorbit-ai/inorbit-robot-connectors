...
def gausium_date_to_inorbit_millis(date: str) -> int:
    """Convert a date from Gausium reports to epoch milliseconds for InOrbit missions."""
...
class MissionTracker:
    ...
    def _complete_mission(
        ...
        planned_cleaning_area = task_report.get("plannedCleaningAreaSquareMeter")
        actual_cleaning_area = task_report.get("actualCleaningAreaSquareMeter")
        consumables = task_report.get("consumablesResidualPercentage", {})

        details = {
            "Report image URI": task_report.get("taskReportPngUri"),
            "Planned cleaning area [m2]": planned_cleaning_area,
            "Actual cleaning area [m2]": actual_cleaning_area,
            "Cleaned area percentage [%]": (
                actual_cleaning_area / planned_cleaning_area * 100
                if actual_cleaning_area and planned_cleaning_area
                else None
            ),
            "Efficiency [m2/h]": task_report.get("efficiencySquareMeterPerHour"),
            "Planned polishing area [m2]": task_report.get("plannedPolishingAreaSquareMeter"),
            "Actual polishing area [m2]": task_report.get("actualPolishingAreaSquareMeter"),
            "Water consumption [L]": task_report.get("waterConsumptionLiter"),
            "Start battery percentage [%]": task_report.get("startBatteryPercentage"),
            "End battery percentage [%]": task_report.get("endBatteryPercentage"),
            "Brush residual percentage [%]": consumables.get("brush"),
            "Filter residual percentage [%]": consumables.get("filter"),
            "Suction blade residual percentage [%]": consumables.get("suctionBlade"),
            "Cleaning mode": self._translate_cleaning_mode(task_report.get("cleaningMode", "")),
        ...
        # State and status derive from the completion percentage (area cleaned) and the
        ...
        state = MissionState.get_for_completion(
            completion_percentage, self._success_threshold, last_progress_bar_percentage
        )
        ...
        # Advance the progress bar to 100% only if the previously published progress was
        # enough to consider the mission finished; otherwise leave it and mark abandoned
