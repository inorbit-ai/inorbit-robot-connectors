...
A robot is reported online to InOrbit while all of the following hold:

- the connector's last `/Robot/UpdatedSince` sweep succeeded within `api_grace_secs` (default 30s),
- the Fleet Manager listed the robot in its last successful sweep,
- the robot's sub-status is not `OutgoingArclConnectionLost`, and
- the robot's status and sub-status are not `Disconnected`, an undocumented value found by probing a live Fleet Manager rather than in Rev C of the manual.

Presence in that list is fleet membership, not reachability: the Integration Toolkit reflects a robot being added to or removed from the fleet within 10 seconds, but a robot that has lost communication stays listed until somebody deregisters it, observed at 17 days on a live Fleet Manager. Connectivity has to be read from the status fields instead, which is what the last two conditions do. A robot that is reachable but faulted (`Fault`, `Lost`, `EstopPressed`, `MotorsDisabled`) stays online and reports `status: ERROR`: it is still reporting, and its pose is what you need to go find it. An ARCL-lost or `Disconnected` robot is reported offline even though the sweep is still succeeding, so FlowCore still holds a current summary for it and `omron_status`, `omron_sub_status` and `status` keep updating to reflect the disconnection. The signature is an offline robot carrying `api_connected: true` and `robot_attached: true`, which no other case produces. Every sub-status that makes a robot offline also maps to `status: ERROR`, so a robot is never reported offline while publishing a status that says it is fine.

Pose and key-values are published only when the `upd.millis` of the items they read has changed, because the Integration Toolkit does not restamp a DataStore item whose value is unchanged (tracked internally as `update_millis`). Mission tracking has no such token to compare against, since the Job and JobSegment streams carry no `upd.millis`, so it publishes only when the mission payload itself changes. Key-values publish in three tiers, each gated on a different precondition because they come from different endpoints with different lifetimes: health (`connector_version`, `api_connected`, `robot_attached`) is the connector's own view and publishes on every execution loop iteration, ungated, so an operator can tell a quiet robot from a broken connector; the fleet summary (`omron_status`, `omron_sub_status`, `status`, `robot_ip`) is FlowCore's own statement from `/Robot/UpdatedSince` and publishes on its own change token whenever the API is connected, regardless of whether the robot itself is online; robot telemetry (`battery_percent`, pose, odometry) is the robot's own data from `/DataStoreValueLatest` and publishes on its change token alone, with no online check on top. A robot whose ARCL link the Fleet Manager has lost keeps reporting its own battery and pose, and that live pose is what somebody needs to go and find it; a robot that has truly dropped answers with nothing, so its token stops advancing and the tier goes quiet on its own. `robot_attached` is omitted while `api_connected` is false, since it is not knowable when the connector cannot reach the API. A known trade of gating pose on change: a robot parked longer than 30 seconds exceeds the edge SDK's distance-accumulation window, so the first pose reported after it moves again is not counted toward InOrbit's distance travelled, roughly one tick of travel per stop-start cycle.
